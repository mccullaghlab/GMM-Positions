...
#@jit
def maximum_likelihood_opt(lnWeights,lnLikelihood,centers,trajData,covar,kabschThresh):
...
#    @jit(locals={ 'self.nFrames': numba.types.int_})#, 'self.nAtoms': numba.types.int_, 'self.nDim': numba.types.int_, 'self.nFeatures': numba.types.int_ })
    def fit(self,trajData):
        ...
        while step < self.maxSteps and logLikeDiff > self.logThresh:
            # Expectation step
            self.lnLikelihood = expectation(self.trajData, self.centers, self.covar)
            # Maximum Likelihood Optimization
            self.centers, self.covar, self.lnWeights, logLikelihoodArray[step] = maximum_likelihood_opt(self.lnWeights, self.lnLikelihood, self.centers, self.trajData, self.covar,self.kabschThresh)
